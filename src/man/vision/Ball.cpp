...
void Ball::preScreenBlobsBasedOnSizeAndColor() {
	...
	for (int i = 0; i < blobs->number(); i++) {
        Blob curBlob = blobs->get(i);
        int ar = curBlob.getArea();
        float perc = static_cast<float>(curBlob.getPixels()) / (float)ar;
        int w = curBlob.width();
        int h = curBlob.height();
        int diam = max(w, h);
        ...
        if (!blobIsBigEnoughToBeABall(w, h)) {
            ...
            if (curBlob.getBottom() + diam <
                horizonAt(curBlob.getLeft())) {
                blobs->init(i);
                if (BALLDEBUG) {
                    cout << "Screened one for horizon problems " << endl;
                    drawBlob(curBlob, WHITE);
                }
            }
                ...
                if (BALLDEBUG) {
                    ...
                    printBlob(curBlob);
                       ...
                       rightHalfColor(curBlob) > minpercent)
            {
                if (BALLDEBUG) {
...
bool Ball::badRatioSC(int w, int h) {
...
bool Ball::distanceMismatchSC(VisualBall * thisBall, estimate e) {
...
bool Ball::badSmallBallSC(int w, int h) {
    ...
    if (h > w) {
        ...
        if (topBlob->getRightTopX() - h > 0) {
            ...
            for (int j = topBlob->getLeftTopY();
                ...
                unsigned char * threshJ= thresh->thresholded[j];
...
float Ball::rightHalfColor(Blob tempobj)
{
    ...
    for (int i = spanY / 2; i < spanY; i++) {
        unsigned char * threshYI = thresh->thresholded[y + i];

        for (int j = 0; j < spanX; j++) {
            int pix = threshYI[x + j];
...
pair<int, int> Ball::scanMidlinesForRoundnessInformation(Blob b) {
    ...
    for (int i = 0; i < h; i++) {
        int pix = thresh->thresholded[y+i][x + w/2];
    ...
    unsigned char * threshYH = thresh->thresholded[y+h/2];
    for (int i = 0; i < w; i++) {
        int pix = threshYH[x + i];
        if (pix == ORANGE || pix == ORANGERED || pix == ORANGEYELLOW) {
            goodPix++;
        } else if (pix != GREY) {
            badPix++;
...
pair<int, int> Ball::scanDiagonalsForRoundnessInformation(Blob b) {
    ...
    for (int i = 0; i < d3; i++) {
        ...
        int pix1 = thresh->thresholded[y+i][x+i];
        if (i < d || (i > d3 - d)) {
            if (pix1 == ORANGE || pix1 == ORANGERED) {
            ...
            if (pix1 == ORANGE || pix1 == ORANGERED || pix1 == ORANGEYELLOW) {
                goodPix++;
            } else if (pix1 != GREY) {
                badPix++;
        ...
        int pix2 = thresh->thresholded[y+i][x+w-i];
        if (i < d || (i > d3 - d)) {
            if (pix2 == ORANGE || pix2 == ORANGERED) {
        ...
        } else if (pix2 == ORANGE || pix2 == ORANGERED ||
                   pix2 == ORANGEYELLOW) {
            goodPix++;
        } else if (pix2 != GREY) {
            badPix++;
...
bool Ball::badSurround(Blob b) {
    ...
    for (int j = 0; j < h && y + j < IMAGE_HEIGHT; j++) {
        unsigned char * threshYJ = thresh->thresholded[y+j];

        for (int i = 0; i < w && x + i < IMAGE_WIDTH; i++) {
            int pix = threshYJ[x + i];
